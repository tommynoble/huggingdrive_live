...
class ModelLoadThread(QThread):
    ...
    def check_memory_requirements(
        self, model_type: str, model_size_gb: float = None
    ) -> tuple[bool, str]:
        ...
        try:
            ...
            if model_size_gb:
                min_ram_gb = max(
                    min_ram_gb, model_size_gb * 2
                )  # Need 2x model size in RAM
                min_vram_gb = max(
                    min_vram_gb, model_size_gb * 1.5
                )  # Need 1.5x model size in VRAM

            # Check RAM
            ram_sufficient = available_ram_gb >= min_ram_gb
            ram_message = (
                f"RAM: {available_ram_gb:.1f}GB available, {min_ram_gb:.1f}GB required"
            )
            ...
            if vram_available:
                ...
                vram_message = (
                    f"VRAM: {vram_gb:.1f}GB available, {min_vram_gb:.1f}GB required"
                )
            else:
                vram_message = "VRAM: CUDA not available, will use CPU"

            # Overall assessment
            overall_sufficient = ram_sufficient and (
                not vram_available or vram_sufficient
            )
            ...
            if (model_path / "config.json").exists():
                ...
                if time.time() - start_time > self.timeout_seconds:
                    self.error_signal.emit(
                        f"Model loading timed out after {self.timeout_seconds} seconds"
                    )
                    return
                ...
                if model_size_gb > 0:
                    self.loading_progress_signal.emit(
                ...
                memory_sufficient, memory_message = self.check_memory_requirements(
                    model_type, model_size_gb
                )
                ...
                if model_type in [
                    ...
                    self.loading_progress_signal.emit(
                        "Loading text classification pipeline..."
                    )
                    self.progress_bar_signal.emit(50)
                    ...
                    self.loading_progress_signal.emit(
                        "Loading text-to-text generation pipeline..."
                    )
                    self.progress_bar_signal.emit(50)
                    ...
                    pipeline_obj = pipeline(
                        "text2text-generation", model=str(model_path)
                    )
                    self.progress_bar_signal.emit(90)
                    self.model_loaded_signal.emit(pipeline_obj, model_type)
                    ...
                    pipeline_obj = pipeline(
                        "image-classification", model=str(model_path)
                    )
                    self.progress_bar_signal.emit(90)
                    self.model_loaded_signal.emit(pipeline_obj, model_type)
                    ...
                    self.error_signal.emit(
                        f"Model type '{model_type}' not supported for testing"
                    )
...
class ModelTestDialog(QDialog):
    ...
    def __init__(self, model_name: str, model_path: str, parent=None):
        ...
        self.chat_history_file = (
            self.cache_dir / f"chat_history_{model_name.replace('/', '_')}.txt"
        )
    ...
    def load_model(self):
        ...
        try:
            ...
            self.model_load_thread.loading_progress_signal.connect(
        ...
        try:
            ...
            if model_type in [
                ...
                self.status_label.setText(
                    "✅ Translation model loaded (use web interface)"
                )
            elif model_type in ["t5", "bart"]:
                self.generate_btn.setEnabled(True)
            ...
            if supports_chat:
                ...
                if not self.load_chat_history_from_cache():
                    ...
                    self.chat_history.append(
                        "🤖 <b>AI Assistant:</b> Hello! I'm ready to chat with you. How can I help you today?"
                    )
                ...
                self.chat_capabilities_label.setStyleSheet(
                ...
                self.chat_history.append(
                    f"🤖 <b>AI Assistant:</b> Hello! I'm {self.model_name}, a {model_type} model. How can I help you today?"
                )

                # Show model loading success
                self.status_label.setText(
                    f"✅ {self.model_name} loaded successfully - Chat enabled!"
                )
    ...
    def unload_model(self):
        ...
        try:
            if self.pipeline:
                ...
                self.chat_capabilities_label.setText(
                    "❌ No model loaded - chat disabled"
                )
                self.chat_capabilities_label.setStyleSheet(
    ...
    def check_memory_usage(self):
        ...
        try:
            ...
            self.memory_info_label.setText(
                f"RAM: {available_gb:.1f}GB/{total_gb:.1f}GB available{vram_info}"
            )
...
"""
    ...
    def translate_text(self):
        ...
        QMessageBox.information(
            self,
            "Translation",
            "Translation functionality is now available in the Web Interface tab.",
        )
    ...
    def send_chat_message(self):
        ...
        try:
            if not self.pipeline:
                self.chat_history.append(
                    "🤖 <b>AI Assistant:</b> Error: Model not loaded. Please load the model first."
                )
                return
            ...
            if hasattr(self.pipeline, "create_chat_completion"):
                ...
                if (
                    hasattr(self.pipeline, "task")
                    and self.pipeline.task == "text2text-generation"
                ):
                    result = self.pipeline(
                        message,
                        max_length=max_length,
            ...
            if new_response:
                ...
                self.chat_history.append(
                    f"🤖 <b>AI Assistant ({timestamp}):</b> {response}"
                )
            else:
                self.chat_history.append(
                    f"🤖 <b>AI Assistant ({timestamp}):</b> I'm not sure how to respond to that."
                )
    ...
    def clean_repetitive_response(self, response: str) -> str:
        ...
        response = re.sub(
            r"\b[a-zA-Z]\s+[a-zA-Z]\s+[a-zA-Z]\s+[a-zA-Z]\b", "", response
        )
        response = re.sub(
            r"\b[A-Za-z]\s+[A-Za-z]\s+[A-Za-z]\s+[A-Za-z]\s+[A-Za-z]\b", "", response
        )
        ...
        response = re.sub(
            r"\b(me|I|you|he|she|it|we|they)\s+\1\b",
            r"\1",
            response,
            flags=re.IGNORECASE,
        )
        response = re.sub(
            r"\b(thanks|thank you)\s+\1\b", r"\1", response, flags=re.IGNORECASE
        )
        response = re.sub(
            r"\b(yeah|yes|no|ok|okay)\s+\1\b", r"\1", response, flags=re.IGNORECASE
        )
        response = re.sub(
            r"\b(haha|lol|XD)\s+\1\b", r"\1", response, flags=re.IGNORECASE
        )
        ...
        for sentence in sentences:
            ...
            for existing in unique_sentences:
                ...
                if len(sentence_words) > 0 and len(existing_words) > 0:
                    similarity = len(sentence_words.intersection(existing_words)) / len(
                        sentence_words.union(existing_words)
                    )
                    if similarity > 0.6:
                        is_duplicate = True
                        break
    ...
    def get_conversation_context(self):
        ...
        for line in lines:
            ...
            elif line.startswith("🤖 AI Assistant:") and not line.startswith(
                "🤖 AI Assistant: <i>Thinking...</i>"
            ):
    ...
    def save_chat_history(self):
        ...
        try:
            ...
            if filename:
                ...
                QMessageBox.information(
                    self, "Success", "Chat history saved successfully!"
                )
        except Exception as e:
    ...
    def load_chat_history(self):
        ...
        try:
            ...
            if filename:
                ...
                QMessageBox.information(
                    self, "Success", "Chat history loaded successfully!"
                )
        except Exception as e:
    ...
    def closeEvent(self, event):
        ...
        if (
            self.gradio_manager
            and self.model_name in self.gradio_manager.get_active_interfaces()
        ):
            self.gradio_manager.stop_interface(self.model_name)
        event.accept()
    ...
    def run(self):
        try:
            ...
            if "unsloth" in self.model_name.lower():
                ...
                self.progress_signal.emit(
                    "This model requires special setup and may not work with standard transformers."
                )
                self.progress_signal.emit(
                    "Consider using a standard model for better compatibility."
                )
            ...
            try:
                ...
                try:
                    ...
                    self.progress_signal.emit(
                        f"Model info retrieved - preparing to download files..."
                    )
                except Exception as e:
                    error_msg = (
                ...
                if hasattr(info, "safetensors") and info.safetensors:
                    ...
                    self.progress_signal.emit(
                        f"Found {total_files} safetensors files to download"
                    )
                elif hasattr(info, "pytorch_model") and info.pytorch_model:
                    total_files = len(info.pytorch_model)
                    self.progress_signal.emit(
                        f"Found {total_files} pytorch model files to download"
                    )
            ...
            self.download_stats_signal.emit(
                f"Download started at {time.strftime('%H:%M:%S')}"
            )
            ...
            self.download_stats_signal.emit(
                f"Download completed in {duration:.1f} seconds"
            )
            ...
            if found_files:
                self.progress_signal.emit(
                    f"Key files downloaded: {', '.join(found_files)}"
                )
...
class DriveManager:
            ...
            try:
                ...
                # Linux: look for /media or /run/media
                    ...
                    if partition.mountpoint.startswith(
                        "/media/"
                    ) or partition.mountpoint.startswith("/run/media/"):
                        usage = psutil.disk_usage(partition.mountpoint)
                        drives.append(
                            {
                                "device": partition.device,
    ...
    def install_model(self, model_name: str, drive_path: str) -> str:
        ...
        model_dir = (
            Path(drive_path) / "huggingface_models" / model_name.replace("/", "_")
        )
        model_dir.mkdir(parents=True, exist_ok=True)

        # Download the model
        snapshot_download(
            repo_id=model_name, local_dir=str(model_dir), local_dir_use_symlinks=False
        )
            ...
            try:
                ...
                try:
                    ...
                    if sys.platform == "darwin":  # macOS
                        ...
                        subprocess.run(
                            ["rmdir", "/s", "/q", model_path], check=True, shell=True
                        )
...
class ModelSearchThread(QThread):
    ...
    def run(self):
        try:
            ...
            for model in models:
                ...
                if any(
                    indicator in model_id_lower
                    for indicator in ["-gguf", "gguf", "ggml", "llama.cpp", "thebloke"]
                ):
            ...
            for model in models:
                ...
                size_info = self.estimate_model_size(
                    getattr(model, "modelId", ""), getattr(model, "tags", [])
                )
    ...
    def estimate_model_size(self, model_id: str, tags: list) -> str:
        ...
        if any(
            ...
            name in model_id_lower
            for name in ["gpt-2", "gpt-3", "llama-2-7b", "llama-3-1b"]
        ):
            return "1.5GB"  # Medium language models
        elif any(
            name in model_id_lower
            for name in ["bert-large", "roberta-large", "distilbert-large"]
        ):
            return "500MB"  # Large BERT models
        elif any(
            name in model_id_lower
            for name in ["bert-base", "roberta-base", "distilbert-base"]
        ):
            return "420MB"  # Base BERT models
        elif any(
            name in model_id_lower for name in ["resnet-50", "resnet-101", "resnet-152"]
        ):
            ...
            name in model_id_lower for name in ["whisper-large", "whisper-medium"]
        ):
    ...
    def get_model_size(self, model_id: str) -> str:
        ...
        try:
            ...
            if hasattr(info, "safetensors") and info.safetensors:
                total_size = sum(
                    file.get("size", 0) for file in info.safetensors.values()
                )
                return self.format_size(total_size)
            elif hasattr(info, "pytorch_model") and info.pytorch_model:
                total_size = sum(
                    file.get("size", 0) for file in info.pytorch_model.values()
                )
                return self.format_size(total_size)
            else:
                return "Unknown"
...
class HuggingDriveGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.ram_usage_label.setStyleSheet(
            "background-color: #f0f0f0; padding: 5px; border-radius: 3px; font-weight: bold;"
        )
        status_bar_layout.addWidget(self.ram_usage_label)

        # App memory usage
        self.app_memory_label = QLabel("📱 App: Loading...")
        self.app_memory_label.setStyleSheet(
            "background-color: #e8f4fd; padding: 5px; border-radius: 3px; font-weight: bold;"
        )
    ...
    def create_right_panel(self) -> QWidget:
        ...
        download_layout.addWidget(
            QLabel("Model Name (search GGUF models on Hugging Face Hub):")
        )
        self.model_name_input = QLineEdit()
        self.model_name_input.setPlaceholderText(
            "Start typing to search GGUF models..."
        )
    ...
    def set_cache_directories(self, drive_path: str):
        ...
        try:
            ...
            self.status_text.append(
                f"Warning: Could not set cache directories: {str(e)}"
            )
        ...
        try:
            if self.current_cache_dir and self.current_cache_dir.exists():
                ...
                if cache_files_removed > 0:
                    self.status_text.append(
                        f"Cleaned up {cache_files_removed} old cache files"
                    )
    ...
    def test_download(self):
        ...
        self.status_text.append(
            "This will test if the basic download functionality works..."
        )
        ...
        if not drive_path and not self.selected_download_folder:
            self.status_text.append(
                "❌ No drive selected - please select a drive first"
            )
            QMessageBox.warning(
                self, "Test Failed", "Please select a drive before testing download"
            )
            return
    ...
    def on_drive_selection_changed(self):
        ...
        if drive_path:
            self.drive_status_label.setText(
                f"✅ Drive selected: {Path(drive_path).name}"
            )
    ...
    def eject_selected_drive(self):
        ...
        if not drive_path:
            QMessageBox.warning(
                self, "No Drive Selected", "Please select a drive to eject"
            )
            return
            ...
            try:
                ...
                if not Path(drive_path).exists():
                    QMessageBox.information(
                        self,
                        "Drive Not Found",
                        f"Drive '{drive_name}' is no longer mounted",
                    )
                ...
                if system == "Darwin":  # macOS
                    ...
                    if result.returncode == 0:
                        self.status_text.append(
                            f"✅ Successfully ejected '{drive_name}'"
                        )
                        QMessageBox.information(
                            self,
                            "Drive Ejected",
                            f"Drive '{drive_name}' has been safely ejected",
                        )
                    ...
                    result = subprocess.run(
                        [
                            "powershell",
                            "-Command",
                            f"Eject-Volume -DriveLetter {drive_path[0]} -Confirm:$false",
                        ],
                        capture_output=True,
                        text=True,
                    )
                    ...
                    result = subprocess.run(
                        ["sudo", "umount", drive_path], capture_output=True, text=True
                    )

                    if result.returncode == 0:
                        self.status_text.append(
                            f"✅ Successfully unmounted '{drive_name}'"
                        )
                        QMessageBox.information(
                            self,
                            "Drive Unmounted",
                            f"Drive '{drive_name}' has been safely unmounted",
                        )
                        self.update_drive_list()
                    else:
    ...
    def check_system_requirements(self):
        ...
        if missing_packages:
            ...
            QMessageBox.warning(
                self,
                "Missing Packages",
                f"Please install: {', '.join(missing_packages)}",
            )
        else:
        ...
        try:
            ...
            self.status_text.append(
                f"System memory: {memory_gb:.1f}GB total, {memory.available / (1024**3):.1f}GB available"
            )

            if memory_gb < 8:
                self.status_text.append(
                    "⚠️ Warning: Less than 8GB RAM may cause issues with large models"
                )
        except:
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Network Error", "No internet connection detected"
            )
    ...
    def perform_model_search(self, query: str, show_popular: bool = False):
        ...
        self.search_thread = ModelSearchThread(
            query, category, None, sort_by, show_popular
        )
    ...
    def on_search_results(self, results: list):
        ...
        for result in results:
            ...
            downloads_str = (
                f"{result['downloads']:,}" if result["downloads"] > 0 else "Unknown"
            )
            ...
            if any(
                ...
                for keyword in ["unsloth", "bnb", "4bit", "8bit", "quantized"]
            ):
    ...
    def choose_download_folder(self):
        ...
        folder = QFileDialog.getExistingDirectory(
            self, "Select Download Folder", drive_path  # Start from the selected drive
        )
        if folder:
    ...
    def update_drive_list(self):
        ...
        for drive in drives:
            self.drive_combo.addItem(
                f"{drive['mountpoint']} ({drive['free_gb']:.1f}GB free)",
                drive["mountpoint"],
            )
    ...
    def scan_for_existing_models(self):
        ...
        if self.selected_download_folder:
            self.status_text.append(
                f"Scanning custom folder: {self.selected_download_folder}"
            )
            custom_models = self.drive_manager.scan_custom_folder_for_models(
                self.selected_download_folder
            )
            for model in custom_models:
        ...
        if custom_folders:
            self.status_text.append(
                f"Scanning {len(custom_folders)} custom folder(s) from config"
            )
            for custom_folder in custom_folders:
                custom_models = self.drive_manager.scan_custom_folder_for_models(
                    custom_folder
        ...
        if models:
            ...
            self.status_text.append(
                "  - /Volumes/[DriveName]/huggingface_models/ (on macOS)"
            )
            self.status_text.append(
                "  - [DriveLetter]:\\huggingface_models\\ (on Windows)"
            )
            self.status_text.append(
                "  - /media/[username]/[DriveName]/huggingface_models/ (on Linux)"
            )
            self.status_text.append(
                "  - Or use 'Choose Download Folder' to select a custom location"
            )
    ...
    def update_model_list(self):
        ...
        self.status_text.append(
            f"Displaying {len(models)} model(s) with size information"
        )
    ...
    def start_download(self):
        ...
        if not drive_path and not self.selected_download_folder:
            QMessageBox.warning(
                self,
                "Error",
                "Please select an external drive or choose a download folder first",
            )
        ...
        if drive_path:
            if not Path(drive_path).exists():
                QMessageBox.warning(
                    self, "Error", f"Selected drive '{drive_path}' no longer exists"
                )
        ...
        if self.selected_download_folder:
            if not Path(self.selected_download_folder).exists():
                QMessageBox.warning(
                    self,
                    "Error",
                    f"Selected folder '{self.selected_download_folder}' no longer exists",
                )
                self.status_text.append(
                    f"❌ Folder {self.selected_download_folder} not found"
                )
                return
            self.status_text.append(
                f"✅ Using custom folder: {self.selected_download_folder}"
            )
        ...
        if self.selected_download_folder:
            target_path = str(
                Path(self.selected_download_folder) / model_name.replace("/", "_")
            )
    ...
    def test_selected_model(self):
        ...
        if model_name in models:
            ...
            QMessageBox.warning(
                self, "Error", f"Model {model_name} not found in installed models"
            )
    ...
    def open_selected_model_folder(self):
        ...
        if model_name in models:
            ...
            try:
                ...
                QMessageBox.critical(
                    self, "Error", f"Failed to open model folder: {str(e)}"
                )
        ...
        if not Path(model_path).exists():
            ...
            if reply == QMessageBox.StandardButton.Yes:
                if self.drive_manager.remove_model(model_name):
                    self.update_model_list()
                    self.status_text.append(
                        f"Removed {model_name} from installed models list"
                    )
                    QMessageBox.information(
                        self,
                        "Success",
                    ...
                    QMessageBox.critical(
                        self, "Error", f"Failed to remove {model_name} from list"
                    )
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                if self.drive_manager.remove_model(model_name):
                    self.update_model_list()
                    self.status_text.append(
                        f"✅ Successfully removed model: {model_name}"
                    )
                    QMessageBox.information(
                        self, "Success", f"Successfully removed model: {model_name}"
                    )
                else:
                    # This shouldn't happen with our improved remove_model method
                    QMessageBox.critical(
                        self, "Error", f"Failed to remove model: {model_name}"
                    )
            except Exception as e:
        ...
        try:
            ...
            if self.selected_download_folder:
                ...
                if models_dir.exists():
                    ...
                    QMessageBox.warning(
                        self, "Error", f"Folder does not exist: {models_dir}"
                    )
            # Otherwise, open the default models directory on the selected drive
                ...
                QMessageBox.warning(
                    self, "Error", "No drive selected and no custom folder set"
                )
        except Exception as e:
    ...
    def export_configuration(self):
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(
                    self, "Success", f"Configuration exported to {file_path}"
                )
    ...
    def update_memory_display(self):
        ...
        try:
            ...
            ram_color = (
                "green" if ram_percent < 70 else "orange" if ram_percent < 90 else "red"
            )
            self.ram_usage_label.setText(
                f"🧠 RAM: {used_ram_gb:.1f}GB/{total_ram_gb:.1f}GB ({ram_percent:.0f}%)"
            )
            self.ram_usage_label.setStyleSheet(
                f"background-color: {ram_color}; color: white; padding: 5px; border-radius: 3px; font-weight: bold;"
            )
            ...
            app_color = (
                ...
                else "orange" if app_memory_mb < 1000 else "red"
            )
            self.app_memory_label.setText(f"📱 App: {app_memory_mb:.0f}MB")
            self.app_memory_label.setStyleSheet(
                f"background-color: {app_color}; color: white; padding: 5px; border-radius: 3px; font-weight: bold;"
            )
            ...
            self.ram_usage_label.setStyleSheet(
                "background-color: red; color: white; padding: 5px; border-radius: 3px; font-weight: bold;"
            )
            self.app_memory_label.setStyleSheet(
                "background-color: red; color: white; padding: 5px; border-radius: 3px; font-weight: bold;"
            )
