...
class GGUFFileFetcherThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress_signal.emit(f"Found {len(gguf_files)} GGUF files. Getting file sizes...")
            
            # Get file sizes for each GGUF file
            file_info_list = []
            for i, file in enumerate(gguf_files):
                self.progress_signal.emit(f"Checking file {i+1}/{len(gguf_files)}: {file}")
            ...
            self.progress_signal.emit(f"Successfully fetched {len(file_info_list)} GGUF files")
