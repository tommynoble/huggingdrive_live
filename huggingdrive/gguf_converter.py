...
class GGUFConverter(QThread):
    ...
    def __init__(self, model_path: str, output_path: Optional[str] = None, 
                 quantization: str = "q4_K_M", context_length: int = 4096):
        super().__init__()
    ...
    def _install_llama_cpp_tools(self) -> bool:
        ...
        try:
            ...
            subprocess.check_call([
                ...
                "llama-cpp-python[conversion]", "--upgrade"
    ...
    def _validate_model(self, model_path: str) -> bool:
        ...
        if not has_weights:
            self.progress_signal.emit("❌ No model weights found (pytorch_model.bin or model.safetensors)")
            return False
    ...
    def run(self):
        try:
            self.progress_signal.emit(f"Starting GGUF conversion of {self.model_path}...")
            ...
            if model_info:
                self.progress_signal.emit(f"Model type: {model_info.get('model_type', 'unknown')}")
                self.progress_signal.emit(f"Architectures: {', '.join(model_info.get('architectures', []))}")
            ...
            if not conversion_script:
                self.progress_signal.emit("llama.cpp conversion tools not found, attempting to install...")
                ...
                conversion_script = self._find_llama_cpp_converter()
                if not conversion_script:
                    raise RuntimeError("Could not find conversion script after installation")
            ...
            self.progress_signal.emit(f"Converting to GGUF format with {self.quantization} quantization...")
            ...
            if process.returncode != 0:
                raise RuntimeError(f"Conversion failed with return code: {process.returncode}")
            
            conversion_time = time.time() - start_time
            
            # Calculate conversion statistics
            try:
                input_size = sum(f.stat().st_size for f in Path(self.model_path).rglob('*') if f.is_file())
                ...
                self.progress_signal.emit(f"Could not calculate conversion statistics: {e}")
...
class GGUFConverterManager:
    ...
    def estimate_conversion_size(self, model_path: str, quantization: str) -> Tuple[float, float]:
        """Estimate input and output sizes for conversion"""
        try:
            input_size = sum(f.stat().st_size for f in Path(model_path).rglob('*') if f.is_file())
            input_mb = input_size / (1024 * 1024)
            
            # Rough estimation based on quantization
            compression_ratios = {
                'q4_0': 4.0, 'q4_1': 4.0, 'q4_K_M': 4.0, 'q4_K_S': 4.0,
                'q5_0': 3.2, 'q5_1': 3.2, 'q5_K_M': 3.2, 'q5_K_S': 3.2,
                'q8_0': 2.0
            }
