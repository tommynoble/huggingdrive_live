...
class DriveManager:
    ...
    def is_valid_model_directory(self, model_dir: Path) -> bool:
        ...
        if any(model_dir.glob("pytorch_model-*.bin")) or any(
        ...
        if model_name in self.config["installed_models"]:
            ...
            try:
                ...
                if os.path.exists(model_path):
                    ...
                    if os.path.exists(model_path):
                        print(
                            f"Warning: Model directory still exists after deletion: {model_path}"
                        )
                        return False
                    else:
                    ...
                    print(
                        f"Model path does not exist (may have been deleted manually): {model_path}"
                    )
                ...
                try:
                    ...
                    self.save_config()
                    print(
                        f"Removed model '{model_name}' from configuration despite file deletion error"
                    )
                    return True
    ...
    def _robust_remove_directory(self, path: str):
        ...
        for root, dirs, files in os.walk(path, topdown=False):
            ...
            for dir in dirs:
                ...
                try:
                    ...
                    try:
                        subprocess.run(
                            ["rmdir", dir_path], capture_output=True, check=False
                        )
                    except Exception:
        ...
        try:
            ...
            # Try using system rmdir command as fallback
            try:
                subprocess.run(["rmdir", path], capture_output=True, check=False)
            except Exception:
