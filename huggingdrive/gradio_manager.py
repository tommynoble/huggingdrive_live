...
class GradioInterfaceManager:
    ...
    def create_chat_interface(self, model_path: str, model_name: str):
        ...
        def chat_with_model(message, history, max_length, temperature):
            try:
                ...
                outputs = pipeline_obj(
                    conversation,
                    max_length=len(pipeline_obj.tokenizer.encode(conversation)) + max_length,
                    temperature=temperature,
                    do_sample=True,
                    pad_token_id=pipeline_obj.tokenizer.eos_token_id,
                    return_full_text=False
    ...
    def create_unified_interface(self, model_path: str, model_name: str, model_type: str):
        ...
        with gr.Blocks(title=f"{model_name} - Web Interface") as interface:
            ...
            def get_model_status():
                if model_name in self.model_pipelines:
                    ...
                    return "🔴 Model Not Loaded", "Model will be loaded automatically when you start using it"
            ...
            def unload_model_from_gradio():
                ...
                if model_name in self.model_pipelines:
                    try:
                        ...
                        return "🔴 Model Unloaded", "Model has been unloaded from memory"
                    except Exception as e:
            ...
            unload_btn.click(
                fn=unload_model_from_gradio,
                outputs=[status_display]
            )
            
            with gr.Tabs():
                # Chat Tab (for conversational models)
                with gr.Tab("💬 Chat", visible=model_type in ["text-generation", "chat"]):
                    gr.Markdown("### Have a conversation with your AI model")
                    gr.Markdown("💡 **Tips:** Start with simple greetings like 'Hello!' or ask questions like 'What can you help me with?' The model works best with clear, specific requests.")
                    chatbot = gr.Chatbot(height=400, label="Chat History")
                    with gr.Row():
                        msg = gr.Textbox(label="Your message", placeholder="Try: 'Hello!', 'What can you help me with?', or 'Tell me a joke'", scale=4)
                    ...
                    with gr.Row():
                        max_length = gr.Slider(minimum=50, maximum=300, value=150, step=10, label="Max Response Length")
                        temperature = gr.Slider(minimum=0.1, maximum=2.0, value=0.8, step=0.1, label="Temperature (0.8 = balanced, 1.2 = creative)")
                    ...
                    def chat_with_model(message, history, max_length, temperature):
                        try:
                            ...
                            if not isinstance(new_history, list):
                                new_history = history_copy + [(message, "Error: Invalid history format")]
                            print(f"DEBUG: Returning history with {len(new_history)} items")
                            return new_history
                    ...
                    def chat_and_clear(message, history, max_length, temperature):
                        if not message.strip():  # Don't process empty messages
                            return history, ""
                        try:
                            result = chat_with_model(message, history, max_length, temperature)
                            # Ensure result is a list of tuples
                            if not isinstance(result, list):
                                result = history + [(message, "Error: Invalid response format")]
                ...
                # Text Generation Tab
                with gr.Tab("✍️ Text Generation", visible=model_type in ["text-generation", "chat"]):
                    gr.Markdown("### Generate text continuations")
                    gr.Markdown("💡 **Tips:** Use presets for quick setup, or fine-tune parameters for custom results. Higher temperature = more creative, lower = more focused.")
                    ...
                    with gr.Row():
                        ...
                        with gr.Column(scale=1):
                            ...
                            # Basic parameters
                            max_length = gr.Slider(minimum=10, maximum=1000, value=150, step=10, label="Max New Tokens")
                            temperature = gr.Slider(minimum=0.1, maximum=2.0, value=0.8, step=0.1, label="Temperature (0.8 = balanced)")
                            ...
                            # Repetition control
                            repetition_penalty = gr.Slider(minimum=0.8, maximum=2.0, value=1.1, step=0.1, label="Repetition Penalty")
                            length_penalty = gr.Slider(minimum=0.5, maximum=2.0, value=1.0, step=0.1, label="Length Penalty")
                            
                            # Advanced options
                            do_sample = gr.Checkbox(label="Use sampling", value=True)
                            early_stopping = gr.Checkbox(label="Early stopping", value=True)
                            num_beams = gr.Slider(minimum=1, maximum=10, value=1, step=1, label="Beam Search (1 = greedy)")
                    ...
                    def generate_text(prompt, max_length, temperature, top_p, top_k, repetition_penalty, length_penalty, do_sample, early_stopping, num_beams):
                        try:
                            if model_name not in self.model_pipelines:
                                pipeline_obj = pipeline("text-generation", model=model_path)
                                self.model_pipelines[model_name] = pipeline_obj
                            ...
                            if num_beams > 1:
                                ...
                                gen_params["do_sample"] = False  # Beam search doesn't use sampling
                    ...
                    generate_btn.click(
                        fn=generate_text,
                        inputs=[input_text, max_length, temperature, top_p, top_k, repetition_penalty, length_penalty, do_sample, early_stopping, num_beams],
                        outputs=output_text
                    )
                    ...
                    creative_btn.click(
                        fn=set_creative,
                        outputs=[max_length, temperature, top_p, top_k, repetition_penalty, length_penalty, do_sample, early_stopping, num_beams]
                    )
                ...
                # Classification Tab
                with gr.Tab("🏷️ Text Classification", visible=model_type == "text-classification"):
    ...
    def launch_interface(self, model_path: str, model_name: str, port: int = 7860) -> str:
