...
class HuggingDriveGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.auth_status_label.setStyleSheet(
            "background-color: #fff3cd; padding: 5px; border-radius: 3px; font-weight: bold; color: #856404;"
        )
        ...
        self.login_btn.setStyleSheet(
            "padding: 5px; background-color: #4CAF50; color: white; border: none; border-radius: 3px;"
        )
        ...
        self.logout_btn.setStyleSheet(
            "padding: 5px; background-color: #f44336; color: white; border: none; border-radius: 3px;"
        )
        ...
        self.memory_timer.start(
            10000
        )  # Update every 10 seconds instead of 5 to reduce UI interference
    ...
    def perform_model_search(self, query: str, show_popular: bool = False):
        ...
        self.search_thread = ModelSearchThread(
            query, limit=10, category="", sort_by=sort_by
        )
    ...
    def is_model_locked(self, model_name: str) -> bool:
        ...
        auth_indicators = [
            ...
            "llama-3",  # Meta models
            "microsoft/phi",
            "microsoft-phi",  # Microsoft models
            "google/gemma",
            "google-gemma",  # Google models
            "anthropic/claude",
            "anthropic-claude",  # Anthropic models
            "openai/gpt",
            "openai-gpt",  # OpenAI models
            "cohere/command",
            "cohere-command",  # Cohere models
            "mistralai/mistral",
    ...
    def on_search_result_selected(self, item: QListWidgetItem):
        ...
        if result_data and isinstance(result_data, dict):
            ...
            self.model_name_input.textChanged.disconnect(
                self.on_model_search_text_changed
            )
            ...
            QTimer.singleShot(
                100,
                lambda: self.model_name_input.textChanged.connect(
                    self.on_model_search_text_changed
                ),
            )
    ...
    def _update_model_selection(self, model_name: str):
        ...
        self.status_text.append(
            "This will download the entire model (config, weights, tokenizer, etc.)."
        )
        self.status_text.append(
            "You can convert it to GGUF format later using the 'Convert to GGUF' button if needed."
        )
    ...
    def start_download(self):
        ...
        if model_name.startswith(("http://", "https://")):
            ...
            if "/" not in model_name:
                QMessageBox.warning(
                    self,
                    "Error",
                    "Please select a model from the search results or enter a valid model URL",
                )
                return
            ...
            if self.is_gguf_repository(model_name):
                self.status_text.append(
                    "🔍 GGUF repository detected! Checking quantization options..."
                )
                dialog = GGUFQuantizationDialog(model_name, self)
                result = dialog.exec()
                if (
                    result == 1 and dialog.selected_file
            ...
            if model_size != "Unknown":
                ...
                class KnownSizeDialog(QDialog):
                    def __init__(self, parent, model_id, model_size):
                        ...
                        info = QLabel(
                            f"<b>Model:</b> {model_id}<br><b>Size:</b> {model_size}"
                        )
                        layout.addWidget(info)

                        # Add link to Hugging Face page
                        link_label = QLabel(
                            f'<a href="https://huggingface.co/{model_id}">🔗 Open on Hugging Face Hub</a>'
                        )
                ...
                class UnknownSizeDialog(QDialog):
                    def __init__(self, parent, model_id, get_file_list_func):
                        ...
                        self.setLayout(layout)
                        info = QLabel(
                            "<b>Model:</b> {}<br><b>Size:</b> <span style='color:red;'>Unknown</span>".format(
                                model_id
                            )
                        )
                        layout.addWidget(info)
                        ...
                        warn = QLabel(
                            "<span style='color:orange;'>The size of this model could not be determined automatically. This is common for private, gated, or complex models. You can still download, or click 'Check Size / Show Files' to see the file list.</span>"
                        )
                        ...
                        self.check_btn.clicked.connect(
                            lambda: self.show_files(model_id, get_file_list_func)
                        )
                            ...
                            try:
                                ...
                                if not files:
                                    self.file_list.setPlainText(
                                        "Could not fetch file list or no files found."
                                    )
                                else:
                                    lines = []
                                    total_size = 0
                                    ...
                                    if total_size > 0:
                                        ...
                                        lines.insert(
                                            0, f"Total estimated size: {total_str}"
                                        )
                                        lines.insert(1, "-" * 50)
                ...
                def get_file_list(model_id):
                    try:
                        ...
                        self.status_text.append(
                            f"⚠️ Could not fetch file list: {str(e)}"
                        )
                        return []
        ...
        if self.selected_download_folder:
            ...
            if not drive_path:
                QMessageBox.warning(
                    self, "Error", "Please select a drive or choose a download folder"
                )
                return
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Could not create download directory: {str(e)}"
            )
            return
    ...
    def update_progress_animation(self):
        ...
        ) % 100  # Slower movement for better visibility
    ...
    def download_finished(self, success: bool, message: str):
        ...
        if success:
            ...
            if downloaded_model_name:
                self.status_text.append(
                    f"✅ Download completed: {downloaded_model_name}"
                )
            ...
            if failed_model_name:
                try:
                    ...
                    self.status_text.append(
                        f"❌ Removed failed model: {failed_model_name}"
                    )
                except Exception as e:
                    self.status_text.append(
                        f"⚠️ Could not remove failed model: {failed_model_name} ({e})"
                    )
                ...
                if failed_model_dir and os.path.exists(failed_model_dir):
                    ...
                    try:
                        ...
                        self.status_text.append(
                            f"🗑️ Deleted failed model directory: {failed_model_dir}"
                        )
                    except Exception as e:
                        self.status_text.append(
                            f"⚠️ Could not delete failed model directory: {failed_model_dir} ({e})"
                        )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                if sys.platform == "darwin":  # macOS
                    ...
                    os.system(
                        f"powershell -Command \"(New-Object -ComObject Shell.Application).NameSpace(17).ParseName('{drive_path}').InvokeVerb('Eject')\""
                    )
                else:  # Linux
...
"""
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to check system requirements: {str(e)}"
            )
    ...
    def update_model_list(self):
        ...
        for model_name, model_info in models.items():
            ...
            try:
                ...
                if config_path.exists():
                    ...
                    if any(
                        ...
                        x in model_type for x in ["bert", "roberta", "distilbert"]
                    ):
                        use = "Classification"
            ...
            if (
                self.currently_downloading
                and model_name == self.currently_downloading.get("name")
            ):
                status_indicator = "⏳ "
        ...
        try:
            ...
            if self.ram_usage_label.text() != new_ram_text:
                ram_color = (
                    "green"
                    if ram_percent < 70
                    else "orange" if ram_percent < 90 else "red"
                )
        ...
        if model_name in models:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                try:
                    ...
                    def robust_remove_directory(path):
                        ...
                        if sys.platform == "darwin":
                            try:
                                ...
                                self.status_text.append(
                                    f"   Warning: Could not remove hidden files: {e}"
                                )
                            ...
                            for file in files:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        self.status_text.append(
                                            f"   Warning: Could not remove file {file_path}: {e}"
                                        )
                        ...
                        try:
                            ...
                            try:
                                subprocess.run(
                                    ["rmdir", path], capture_output=True, check=False
                                )
                            except Exception:
                                self.status_text.append(
                                    f"   Warning: Could not remove root directory {path}: {e}"
                                )
                    ...
                    if not success:
                        self.status_text.append(
                            "⚠️ Warning: Could not update model configuration"
                        )
                    ...
                    if os.path.exists(model_path):
                        ...
                        if os.path.exists(model_path):
                            ...
                            self.status_text.append(
                                f"❌ Model deletion incomplete - some files remain at {model_path}"
                            )
                        else:
                            QMessageBox.information(
                                self,
                                "Model Removed",
                                f"Model '{model_name}' was successfully deleted.",
                            )
                            self.status_text.append(
    ...
    def convert_selected_model_to_gguf(self):
        ...
        if not current_item:
            QMessageBox.warning(
                self, "Error", "Please select a model to convert to GGUF"
            )
            return
        ...
        if model_name in models:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                self.conversion_thread.progress_bar_signal.connect(
                    self.update_progress_bar
                )
                self.conversion_thread.conversion_stats_signal.connect(
                ...
                self.convert_to_gguf_btn.setEnabled(False)
                self.status_text.append(
                    f"🚀 Starting GGUF conversion of {model_name}..."
                )
        else:
            QMessageBox.warning(
                self, "Error", f"Model {model_name} not found in installed models"
    ...
    def stop_download(self):
        ...
        if (
            ...
            self.progress_bar.setVisible(False)
            QMessageBox.information(
                self, "Download Stopped", "The download was stopped."
            )
    ...
    def start_api_server(self):
        ...
        if not current_item:
            QMessageBox.warning(
                self, "Error", "Please select a model to serve via API."
            )
            return
        model_name = current_item.data(Qt.ItemDataRole.UserRole)
        models = self.drive_manager.get_installed_models()
        ...
        for _ in range(20):
            ...
            QMessageBox.warning(
                self, "Error", "Could not find a free port for the API server."
            )
            return
...
'''.format(
            model_path, self.api_server_port
        )
        import subprocess
        import tempfile
        import os
    ...
    def show_paste_gguf_url_dialog(self):
        ...
        class PasteUrlDialog(QDialog):
            def __init__(self, parent=None):
                ...
                layout.addWidget(
                    QLabel(
                        "Example: https://huggingface.co/TheBloke/Llama-2-13B-chat-GGUF/resolve/main/..."
                    )
                )
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            try:
                ...
                if "resolve" in path_parts and url.endswith(".gguf"):
                    ...
                    self.update_status(
                        f"✅ Direct GGUF file detected: {filename}{size_info}"
                    )
                    self.update_status(
                        "You can now click Download to start downloading the file"
                    )
                ...
                if "huggingface.co" in parsed.netloc and len(path_parts) >= 3:
                    ...
                    if len(path_parts) > 4 and path_parts[-1].endswith(".gguf"):
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                if total_size > 0:
                                    ...
                                    response = requests.get(
                                        direct_url, stream=True, timeout=30
                                    )
                            ...
                            self.status_text.append(
                                f"Note: Could not fetch file size: {str(e)}"
                            )
                            size_info = " (size unknown)"
                        ...
                        # Show helpful messages
                        self.update_status(
                            f"✅ GGUF file found in repository: {file_path}{size_info}"
                        )
                        self.update_status(
                            "You can now click Download to start downloading the file"
                        )
                ...
                QMessageBox.warning(
                    self,
                    "Error",
                    "Please use a valid Hugging Face model URL (https://huggingface.co/...)",
                )
            except Exception as e:
    ...
    def update_auth_status(self):
        ...
        if self.auth_manager.is_authenticated():
            ...
            self.auth_status_label.setStyleSheet(
                "background-color: #d4edda; padding: 5px; border-radius: 3px; font-weight: bold; color: #155724;"
            )
            # Show green login button when logged in
            self.login_btn.setText("✅ Logged In")
            self.login_btn.setStyleSheet(
                "padding: 5px; background-color: #28a745; color: white; border: none; border-radius: 3px; font-weight: bold;"
            )
    ...
    def logout_huggingface(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.update_auth_status()
            QMessageBox.information(
                self, "Logged Out", "Successfully logged out from Hugging Face Hub"
            )
    ...
    def is_gguf_repository(self, repo_id: str) -> bool:
        ...
        try:
            ...
            return (
                len(gguf_files) > 1
            )  # Multiple GGUF files indicate quantization options
        except Exception as e:
    ...
    def download_specific_gguf_file(self, repo_id: str, filename: str):
        ...
        try:
            ...
            if self.selected_download_folder:
                ...
                if not drive_path:
                    QMessageBox.warning(
                        self,
                        "Error",
                        "Please select a drive or choose a download folder",
                    )
                    return
                target_path = str(Path(drive_path) / "huggingface_models")
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to start GGUF download: {str(e)}"
            )
        ...
        try:
            ...
            if not (model_path / "config.json").exists():
                self.finished_signal.emit(
                    False, "Model does not appear to be in Hugging Face format"
                )
                return
            ...
            if any("llama" in arch for arch in architectures) or "llama" in model_type:
                ...
                any("mistral" in arch for arch in architectures)
            ...
            if not conversion_script_path:
                self.finished_signal.emit(
                    False,
                    "llama.cpp conversion script not found. Please download llama.cpp and place the conversion scripts in the app directory. See: https://github.com/ggerganov/llama.cpp#converting-models",
                )
                return
            ...
            cmd = [
                ...
                str(model_path),
                "--outfile",
                str(output_dir / f"{model_path.name}.gguf"),
                "--outtype",
                "q4_0",
            ]
            ...
            if process.returncode == 0:
                self.finished_signal.emit(
                    True, f"Conversion successful! GGUF model saved to: {output_dir}"
                )
            else:
                self.finished_signal.emit(
                    False, f"Conversion failed with return code: {process.returncode}"
                )
        except Exception as e:
...
class GGUFQuantizationDialog(QDialog):
    def __init__(self, repo_id, parent=None):
        ...
        title_label.setStyleSheet(
            "font-size: 14px; font-weight: bold; margin-bottom: 10px; color: white;"
        )
        ...
        if recommendation:
            ...
            rec_label.setStyleSheet(
                "color: #4CAF50; font-size: 12px; margin-bottom: 10px; padding: 8px; background-color: rgba(76, 175, 80, 0.1); border-radius: 5px; border-left: 3px solid #4CAF50;"
            )
        ...
        self.table.setHorizontalHeaderLabels(
            ["Quantization", "Size", "Quality", "Description"]
        )
        ...
        self.info_label.setStyleSheet(
            "background-color: rgba(248, 249, 250, 0.1); padding: 10px; border-radius: 5px; margin: 10px 0; border: 1px solid #ddd;"
        )
    ...
    def populate_quantizations(self, repo_id):
        ...
        try:
            ...
            for i, filename in enumerate(gguf_files):
                ...
                # Get description
                description = quant_descriptions.get(
                    quant_type, f"{quant_type} quantization"
                )
