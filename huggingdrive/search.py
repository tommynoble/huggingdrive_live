...
class ModelSearchThread(QThread):
        ...
        try:
            ...
            for i, model in enumerate(models):
                self.progress_signal.emit(
                    f"Processing model {i+1}/{len(models)}: {model.modelId}"
                )
                ...
                if self.use_actual_sizes:
                    ...
                    size_info = self.estimate_model_size(
                        model.modelId, getattr(model, "tags", [])
                    )
    ...
    def estimate_model_size(self, model_id: str, tags: list) -> str:
        ...
        if any(
            ...
            large_model in model_lower
            for large_model in ["llama-2-7b", "mistral-7b", "falcon-7b", "bloom-7b"]
        ):
            return "7GB"
        elif any(
            large_model in model_lower
            for large_model in ["llama-2-3b", "mistral-3b", "falcon-3b", "bloom-3b"]
        ):
            return "3GB"
        elif any(
            large_model in model_lower
            for large_model in ["llama-2-1b", "mistral-1b", "falcon-1b", "bloom-1b"]
        ):
            return "1GB"
