...
class ModelDownloader(QThread):
    ...
    def __del__(self):
        ...
        try:
            ...
            if hasattr(self, '_cleanup_required') and self._cleanup_required and hasattr(self, '_partial_file') and self._partial_file:
                try:
        ...
        try:
            with open(target_file, 'wb') as f:
                for chunk_id in range(total_chunks):
                    ...
                    try:
                        chunk_id, chunk_data = self.write_queue.get(timeout=5)  # 5 second timeout
                        f.write(chunk_data)
                        ...
                        if self.total_size > 0:
                            ...
                            if elapsed > 0:
                                speed = self.total_downloaded / (1024 * 1024 * elapsed)  # MB/s
                        ...
                        if not self._should_stop:
                            self.progress_signal.emit(f"Error writing chunk {chunk_id}: {str(e)}")
                        return
    ...
    def _download_file(self, url: str, target_path: str):
        ...
        try:
            ...
            if any(error_page in response.url.lower() for error_page in ['error', '404', '403', 'unauthorized']):
            ...
            if 'text/html' in content_type or 'text/plain' in content_type:
                raise ValueError(f"URL returns HTML/text instead of a file (content-type: {content_type})")
            ...
            if self.total_size == 0:
                ...
                self.progress_signal.emit("Server doesn't support partial downloads, using direct download...")
                response = requests.get(final_url, headers=headers, stream=True, timeout=60, allow_redirects=True)
                response.raise_for_status()
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for start, end, chunk_id in chunks:
                    if self._should_stop:
                        return False
                    futures.append(
                        executor.submit(self._download_chunk, final_url, start, end, chunk_id)
                    )
            ...
            try:
                with open(target_file, 'rb') as f:
                    magic = f.read(4)
                    if magic != b'GGUF':
                        raise ValueError(f"Downloaded file is not a valid GGUF model file (detected type: {magic})")
                self.progress_signal.emit("✅ File verification passed - valid GGUF file")
            except Exception as e:
                # Clean up the invalid file
                if os.path.exists(target_file):
                    os.remove(target_file)
                raise ValueError(f"Downloaded file is not a valid GGUF model file: {str(e)}")
    ...
    def check_auth_requirements(self, url: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if response.status_code == 401:
                return True, "🔒 Model is private or requires authentication (401 Unauthorized)"
            elif response.status_code == 403:
                return True, "🔒 Access forbidden - model may be private (403 Forbidden)"
        ...
        try:
            ...
            try:
                ...
                if "403" in error_msg or "forbidden" in error_msg.lower():
                    return False, f"🔒 Access denied to {model_name}. You need to request access."
    ...
    def run(self):
        try:
                ...
                if '/' in download_url:
                    ...
                    if len(parts) >= 2:
                        ...
                        if len(parts) > 2 and parts[-1].endswith('.gguf'):
                            ...
                            self.progress_signal.emit(f"Repository provided: {repo_name}")
                            self.progress_signal.emit("Downloading entire model repository...")
                            ...
                            token = os.environ.get('HUGGING_FACE_HUB_TOKEN')
                            if token:
                                self.progress_signal.emit("✅ Authentication token found")
                            else:
                                self.progress_signal.emit("⚠️ No authentication token found")
                            ...
                            try:
                                ...
                                if "403" in error_msg or "forbidden" in error_msg.lower():
                                    if token:
                                        ...
                                        self.progress_signal.emit("💡 You're logged in, but need to request access to this model")
                                        self.progress_signal.emit("   Visit: https://huggingface.co/" + repo_name)
                                        self.progress_signal.emit("   Click 'Request access' and wait for approval")
                                    else:
                                        auth_message = f"🔒 Access forbidden - {repo_name} requires authentication"
                                        self.progress_signal.emit(auth_message)
                                        self.progress_signal.emit("💡 Please log in using the '🔑 Login to HF' button")
                                    self.finished_signal.emit(False, auth_message)
                                    return
                                elif "404" in error_msg or "not found" in error_msg.lower():
                                    auth_message = f"❌ Repository not found: {repo_name}"
                                    self.progress_signal.emit(auth_message)
                                    self.finished_signal.emit(False, auth_message)
                                    return
                                else:
                                    raise ValueError(f"Failed to access repository: {error_msg}")
                            
                            # Use huggingface_hub to download the model
                            try:
                                from huggingface_hub import snapshot_download
                                self.progress_signal.emit("Using huggingface_hub to download model...")
                                ...
                                try:
                                    ...
                                    token = os.environ.get('HUGGING_FACE_HUB_TOKEN')
                                    if token:
                                        self.progress_signal.emit("Using authentication token for download...")
                                    ...
                                    if "403" in error_msg or "forbidden" in error_msg.lower():
                                        auth_message = f"🔒 Access forbidden - {repo_name} requires authentication"
                                        self.progress_signal.emit(auth_message)
                                        self.progress_signal.emit("💡 Tip: Try searching for public models instead")
                                        self.progress_signal.emit("   Popular public models: TinyLlama, Rocket-3B, StableLM")
                                        self.finished_signal.emit(False, auth_message)
                                        return
                                    else:
                                        raise e
                                ...
                                self.progress_signal.emit("✅ Model downloaded successfully!")
                                ...
                                try:
                                    ...
                                    if duration > 0:
                                        ...
                                        total_size = sum(f.stat().st_size for f in Path(self.target_path).rglob('*') if f.is_file())
                                    ...
                                    self.progress_signal.emit(f"Could not calculate final statistics: {e}")
                                
                                self.progress_bar_signal.emit(100)
                                self.finished_signal.emit(True, f"Model downloaded to {self.target_path}")
                                return
                                
                            except ImportError:
                                self.progress_signal.emit("huggingface_hub not available, falling back to manual download...")
                                # Fallback: try to find and download individual files
                                raise ValueError(f"Please install huggingface_hub: pip install huggingface_hub")
                            except Exception as e:
            ...
            if requires_auth:
                self.progress_signal.emit(auth_message)
                self.progress_signal.emit("💡 Tip: Try searching for public GGUF models instead")
                self.progress_signal.emit("   Popular public models: TinyLlama, Rocket-3B, StableLM")
                self.finished_signal.emit(False, auth_message)
                return
            
            self.progress_signal.emit("✅ Model is publicly accessible - starting download...")
            ...
            if os.path.isfile(self.target_path):
                raise ValueError(f"Target path {self.target_path} is a file, but should be a directory")
            
            # Check if file already exists
            if os.path.exists(target_file):
                self.progress_signal.emit(f"File {filename} already exists, skipping download")
            ...
            try:
                ...
                self.progress_signal.emit(f"Could not calculate download statistics: {e}")
    ...
    def _find_best_gguf_file(self, repo_name: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    files = response.json()
                    gguf_files = [f['path'] for f in files if f['path'].endswith('.gguf')]
