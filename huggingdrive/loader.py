...
class ModelLoadThread(QThread):
    ...
    def __init__(self, model_path: str, model_name: str):
        ...
        if any(
            large_model in model_name.lower()
            for large_model in ["llama", "mistral", "falcon", "bloom", "gpt-neo"]
        ):
    ...
    def estimate_model_size(self, model_path: str) -> float:
        ...
        try:
            ...
            self.loading_progress_signal.emit(
                f"Warning: Could not estimate model size: {e}"
            )
            return 0.0  # Return as float

    def check_memory_requirements(
        self, model_type: str, model_size_gb: float = 0.0
    ) -> tuple[bool, str]:
        ...
        try:
            ...
            if model_size_gb > 0.0:
                ...
                if model_size_gb > 10.0:  # Very large models (like Llama 8B+)
                    min_ram_gb = (
                        model_size_gb * 3.0
                    )  # Need 3x model size for very large models
                elif model_size_gb > 5.0:  # Large models
                    min_ram_gb = (
                        model_size_gb * 2.5
                    )  # Need 2.5x model size for large models
                else:
    ...
    def run(self):
        try:
            ...
            if model_path.is_file() and model_path.suffix.lower() == ".gguf":
                ...
                is_gguf = True
                self.loading_progress_signal.emit(
                    f"Loading GGUF file directly: {gguf_path}"
                )
            # Case 2: Directory containing a GGUF file
                ...
                if (model_path / "config.json").exists():
                    ...
                    if gguf_files:
                        ...
                        self.loading_progress_signal.emit(
                            f"Selected largest GGUF file: {gguf_path} ({gguf_path.stat().st_size / (1024**3):.2f} GB)"
                        )
                        is_gguf = True
                    else:
                        is_gguf = False
                        self.error_signal.emit(
                            f"No GGUF files found in directory: {model_path}"
                        )
                        return
            else:
                self.error_signal.emit(
                    f"Model path must be either a .gguf file or a directory containing a GGUF file or config.json. Got: {model_path}"
                )
                return
            ...
            if is_gguf:
                ...
                try:
                    self.loading_progress_signal.emit(
                        "Loading GGUF model with llama-cpp-python..."
                    )
                    ...
                    class GGUFPipeline:
                        def __init__(
                            self, model_path, use_gpu=False, progress_callback=None
                        ):
                            ...
                            if not model_path.is_file():
                                raise ValueError(
                                    f"Path exists but is not a file: {model_path}"
                                )
                            ...
                            try:
                                with open(model_path, "rb") as f:
                                    ...
                                    if magic != b"GGUF":
                                        raise ValueError(
                                            f"Invalid GGUF file: expected magic number 'GGUF', got {magic}"
                                        )
                                if progress_callback:
                                    progress_callback("GGUF file validation passed")
                            except Exception as validation_error:
                                raise ValueError(
                                    f"GGUF file validation failed: {validation_error}"
                                )
                            ...
                            try:
                                ...
                                if progress_callback:
                                    progress_callback(
                                        f"Loading model from {model_path} with {n_threads} threads..."
                                    )
                                ...
                                try:
                                    ...
                                    if progress_callback:
                                        progress_callback(
                                            f"First initialization failed: {init_error}"
                                        )
                                        progress_callback(
                        ...
                        def __call__(self, prompt, **kwargs):
                            if not hasattr(self, "llm"):
                                raise RuntimeError(
                                    "Model not initialized or already cleaned up"
                                )
                    ...
                    if use_gpu:
                        ...
                        self.loading_progress_signal.emit(
                            f"Found GPU: {gpu_name} with {vram_gb:.1f}GB VRAM"
                        )
                    else:
                        self.loading_progress_signal.emit(
                    ...
                    self.loading_progress_signal.emit(
                        f"Initializing model {'with GPU acceleration' if use_gpu else 'on CPU'}..."
                    )
                    pipeline_obj = GGUFPipeline(
                        gguf_path,
                        use_gpu=use_gpu,
                        progress_callback=self.loading_progress_signal.emit,
                    )
                        ...
                        if use_gpu:
                            ...
                            subprocess.check_call(
                                [
                                    ...
                                    "--upgrade",
                                    "--extra-index-url",
                                    "https://download.pytorch.org/whl/cu118",  # Use CUDA 11.8
                                ],
            ...
            if (model_path / "config.json").exists():
                ...
                try:
                    ...
                    self.loading_progress_signal.emit(
                        "Copying model to system memory for faster access..."
                    )
                    import shutil
                ...
                self.error_signal.emit(
                    "Model path must be either a .gguf file or a directory containing a GGUF file or config.json"
                )
