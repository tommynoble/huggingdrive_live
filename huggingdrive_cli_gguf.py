...
Examples:
  ...
  %(prog)s /path/to/model --validate
    ...
    parser.add_argument(
        "model_path", nargs="?", help="Path to the Hugging Face model directory"
    )
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate model for conversion and exit"
    )

    parser.add_argument(
        "--estimate-size", action="store_true", help="Estimate conversion size and exit"
    )
    ...
    if args.estimate_size:
        input_mb, output_mb = manager.estimate_conversion_size(
            str(model_path), args.quantization
        )
